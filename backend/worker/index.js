...
const { exportJobsToExcel } = require("../services/excelExport");
const { parse } = require("path");

// Your Google Sheet ID (from the URL: docs.google.com/spreadsheets/d/{SHEET_ID})
...
const sendMail = async (mailOptions) => {
  return transporter
    ...
    .then((info) => {
      console.log(
        `${moment().format("DD/MM/YYYY")} - Done, message was sent to `,
        mailOptions.to,
      );
    })
    .catch((error) => {
...
const newJobNotification = cron.schedule(
  "21 15 * * *",
  async () => {
    ...
    try {
      ...
      for (const customer of customers) {
        ...
        for (const company of companies) {
          ...
          console.log('websiteJobs', websiteJobs);
          ...
          if (companyNewJobsCount > 0) {
            ...
            if (websiteJobs.length > 0) {
              ...
              // Export website jobs to Excel file
              try {
                totalWebsiteJobs = totalWebsiteJobs.concat(websiteJobs);
              } catch (error) {
                console.error('Failed to export Website jobs to Excel:', error.message);
              }
            ...
            if (linkedinJobs.length > 0) {
              ...
              // Export LinkedIn jobs to Excel file
              try {
                const filePath = exportJobsToExcel('LinkedIn Jobs', linkedinJobs);
                console.log('Exported LinkedIn Jobs to', filePath);
              } catch (error) {
                console.error('Failed to export LinkedIn jobs to Excel:', error.message);
              }
        ...
        const filePath = exportJobsToExcel('Website Jobs', totalWebsiteJobs);
        console.log('Exported Website Jobs to', filePath);
      }
      ...
      for (const company of companies) {
        ...
        if (period.at(-1) === "h") {
          ...
          if (!lastScan || now - lastScan >= frequencyInMs) {
            ...
            const res = await Company.scanCompany({
              id: userId,
              companyId: companyId,
            });
